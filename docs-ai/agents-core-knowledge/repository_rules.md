...
Do not add real API keys, passwords, tokens, or other secrets to code, config files, or tests. Only documented variable names are allowed in Git. Real secrets are passed via environment variables, secret managers, or local `.env` files that are ignored by `.gitignore`.
